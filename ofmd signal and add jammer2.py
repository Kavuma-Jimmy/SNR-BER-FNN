...
# Feed-Forward Neural Network Suppression
...
# Pre-IFFT Jammer Suppression (Notch Filter in Time Domain)
pre_ifft_filtered_signal = apply_notch_filter(ofdm_time_cp_jammed, b, a)

# Post-IFFT Jammer Suppression (Notch Filter in Frequency Domain after IFFT)
def apply_ifft_and_filter(ofdm_freq, b, a):
    ofdm_time = np.fft.ifft(ofdm_freq, axis=1)
    ofdm_time_cp = add_cyclic_prefix(ofdm_time, cp_len)
    filtered_signal = apply_notch_filter(ofdm_time_cp, b, a)
    return filtered_signal

post_ifft_filtered_signal = apply_ifft_and_filter(ofdm_freq_jammed, b, a)
...
# Plotting the results in the specified order
...
plot_signal(pre_ifft_filtered_signal, 'Pre-IFFT Filtered (Time Domain)')
plot_signal(post_ifft_filtered_signal, 'Post-IFFT Filtered (Time Domain)')
